...
- `chapter04.tex` — **Experiments & Results** (MOSTLY WRITTEN): Contains ALL implementation details and results. Hardware platform (FR3 + Agile Hand specs from brochure), simulation environment (Isaac Lab DirectRLEnv, PhysX config tables), object set (13 VisDex objects), episode structure, teacher obs/action space tables, reward terms table with weights, network architecture (LSTM 1024), PPO hyperparameters table, ADR parameter ranges table, student architecture (ResNet-18 + cross-attention + LSTM 512), SafeDAgger distillation config (350k iters, 24 envs), evaluation protocol (GSR/UER/TQR + physics_instability category). Results: three teacher policies (teacher 9/10/11) with eval data, complete DAgger vs SafeDAgger comparison (runs 1c, 2a, 2b, 3a, 3b) with concrete numbers (best student: 59% GSR with teacher 11). Key findings: teacher quality dominates distillation method, vanilla DAgger converges 3.5x faster, sim2real teacher produces dramatically safer students. Safety analysis with failure mode breakdown. Sim2real curriculum design and results (teacher 11 ADR-14). Ablations: envs, penalty, teacher strength, LSTM checkpoint resume, training collapse at 700k. Remaining placeholders: some figures, GPU specs, wall-clock times, per-object breakdown.
...
- Chapter 4 is **mostly written** with concrete data from experiment logs (exp_02 through exp_06). Data sources: exp_02 (teacher training), exp_03 (sim2real curriculum, teacher 11), exp_04 (distillation runs 1c/2a/2b/3a/3b), exp_05 (eval tooling), exp_06 (teacher vs student gap). Key results: three teacher policies evaluated (teacher 9/10/11), complete DAgger vs SafeDAgger comparison with real numbers (best student: run3a at 59% GSR). Key findings documented: teacher quality dominates distillation method, vanilla DAgger converges 3.5x faster, sim2real teacher produces safer students (harmful collision 19% vs 41%), extended training can collapse (run2b). Remaining placeholders: some figures, GPU model/wall-clock times, per-object eval breakdown.
