...
Despite this potential, high degree-of-freedom (DoF) hands remain largely confined to research laboratories.
...
Learning-based methods, particularly deep reinforcement learning (RL), have recently demonstrated impressive dexterous manipulation capabilities in simulation~\cite{Handa2024, Lin2025}.
...
\begin{enumerate}
    \item A \textbf{privileged teacher policy} is trained via RL (PPO) in a GPU-accelerated physics simulator with access to ground-truth object poses, joint states, and contact information.
...
The original DextrAH-RGB work employs Fabric-Guided Policies (FGPs) built on geometric fabrics~\cite{Ratliff2023} and uses DAgger for distillation.
This thesis departs from the original approach in two key ways:
\begin{itemize}
    \item The teacher policy is trained as a standard RL policy \textbf{without geometric fabrics}, using custom reward functions in the Isaac Lab manager-based environment with the Agile Hand mounted on a Franka Research~3 arm.
...
\begin{enumerate}
    \item \textbf{Dexterous grasping environment in Isaac Lab:} A manager-based Isaac Lab environment for training arm-hand grasping policies with the Agile Hand and Franka Research~3, including custom reward shaping and asset integration.
    ...
    \item \textbf{SafeDAgger-based distillation:} A comparative study of DAgger versus SafeDAgger for distilling the privileged teacher into a vision-based student, evaluating both task performance (grasp success rate) and training safety (number of unsafe episodes).
\end{enumerate}
